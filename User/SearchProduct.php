...
<div aria-live="polite" aria-atomic="true" class="position-fixed bottom-0 end-0 p-3" style="z-index: 11">
    ...
    <div class="filters" id="sidebar">
        ...
        ?>
            <div class="form-check">
                <input class="form-check-input" type="checkbox" value="<?php echo $data['category_id']; ?>" id="category<?php echo $data['category_id']; ?>" name="category" onchange="getgrade(this.value),getSearch()">
    ...
    <script>
        ...
        function getgrade(category) {
    $.ajax({
        url: "../Asset/AjaxPages/AjaxSearchGrade.php?category=" + category,
        method: "GET",
        success: function (response) {
            document.getElementById("sel_grade").innerHTML = response;
        }
    });
}
        function getSearch() {
            const category = getSelectedCheckboxes("category");
            const grade = getSelectedCheckboxes("grade");
           
            const searchKeyword = document.getElementById("search-field").value;

            $.ajax({
                url: "../Asset/AjaxPages/AjaxSearchProduct.php?search="+searchKeyword+"&category="+category+"&grade="+grade,
                method: "GET",
                success: function (response) {
